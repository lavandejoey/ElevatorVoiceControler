...
def rasr_example():
    ...
    request.set_vad_tail(800)  # 设置有效尾部，[0, 3000]， 默认500
...
def vertical_motor_reset():
    while not gpio.input(vertical_reset_switch):
        ...
        forward_motor(delay=0.00002, steps=5, port=vertical_motor_ports)
...
def horizon_motor_reset():
    while not gpio.input(horizon_reset_switch):
        backward_motor(delay=0.00002, steps=5, port=horizontal_motor_ports)
...
def horizontal_act():
    ...
    if step > 0:  # right
        forward_motor(delay=0.00002, steps=step, port=horizontal_motor_ports)
    elif step < 0:  # left
        step = abs(step)
        backward_motor(delay=0.00002, steps=step, port=horizontal_motor_ports)
    h_flag = 1
...
def vertical_act():
    ...
    step = vertical_step
    if step > 0:  # up
        forward_motor(delay=0.00002, steps=step, port=vertical_motor_ports)
    elif step < 0:  # down
        step = abs(step)
        backward_motor(delay=0.00002, steps=step, port=vertical_motor_ports)
    v_flag = 1
